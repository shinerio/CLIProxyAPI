...
func (m configTabModel) parseConfig(cfg map[string]any) []configField {
	...
	// TLS / SSL
	tlsCfg := proxyconfig.TLSConfig{}
	if tlsRaw, ok := cfg["tls"].(map[string]any); ok {
		tlsCfg.Enable = getBool(tlsRaw, "enable")
		tlsCfg.Cert = getString(tlsRaw, "cert")
		tlsCfg.Key = getString(tlsRaw, "key")
	}
	fields = append(fields, configField{"TLS Enable", "tls/enable", "bool", fmt.Sprintf("%v", tlsCfg.Enable), nil})
	fields = append(fields, configField{"TLS Cert Path", "tls/cert", "string", tlsCfg.EffectiveCertPath(), tlsCfg.Cert})
	fields = append(fields, configField{"TLS Key Path", "tls/key", "string", tlsCfg.EffectiveKeyPath(), tlsCfg.Key})
...
func fieldSection(apiPath string) string {
	...
	if strings.HasPrefix(apiPath, "tls/") {
		return "TLS / SSL"
	}
	switch apiPath {
