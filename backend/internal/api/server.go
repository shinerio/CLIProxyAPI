...
type Server struct {
	...
	// localHTTPServer is an optional secondary plain HTTP listener.
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	engine.Use(s.preferLocalFrontendMiddleware())
	...
	if cfg.LocalHTTP.Enable {
		s.localHTTPServer = &http.Server{
			Addr:    listenAddr(cfg.LocalHTTP.EffectiveHost(), cfg.LocalHTTP.EffectivePort()),
			Handler: engine,
		}
	}
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.GET("/tls/enable", s.mgmt.GetTLSEnable)
		mgmt.PUT("/tls/enable", s.mgmt.PutTLSEnable)
		mgmt.PATCH("/tls/enable", s.mgmt.PutTLSEnable)
		mgmt.GET("/tls/cert", s.mgmt.GetTLSCert)
		mgmt.PUT("/tls/cert", s.mgmt.PutTLSCert)
		mgmt.PATCH("/tls/cert", s.mgmt.PutTLSCert)
		mgmt.GET("/tls/key", s.mgmt.GetTLSKey)
		mgmt.PUT("/tls/key", s.mgmt.PutTLSKey)
		mgmt.PATCH("/tls/key", s.mgmt.PutTLSKey)
...
func (s *Server) preferLocalFrontendMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !s.isLocalFrontendCandidatePath(requestPath) {
			c.Next()
			return
		}

		filePath := managementasset.ResolveLocalAssetPath(s.configFilePath, requestPath)
...
func (s *Server) isLocalFrontendCandidatePath(requestPath string) bool {
	requestPath = path.Clean("/" + strings.TrimSpace(requestPath))
	if requestPath == "/" {
		return true
	}

	if requestPath == "/management.html" {
		return true
	}
...
func (s *Server) serveManagementControlPanel(c *gin.Context) {
	...
	if _, err := os.Stat(filePath); err != nil {
		if os.IsNotExist(err) {
			log.WithField("path", filePath).Warn("local management asset not found")
			c.AbortWithStatus(http.StatusNotFound)
			return
		} else {
...
func (s *Server) Start() error {
	...
	primaryListener, err := net.Listen("tcp", s.server.Addr)
	if err != nil {
		return fmt.Errorf("failed to listen on %s: %w", s.server.Addr, err)
	}
	var localHTTPListener net.Listener
	if s.localHTTPServer != nil {
		localHTTPListener, err = net.Listen("tcp", s.localHTTPServer.Addr)
		if err != nil {
			_ = primaryListener.Close()
			return fmt.Errorf("failed to listen on %s: %w", s.localHTTPServer.Addr, err)
		}
	}
	...
	if useTLS {
		cert = s.cfg.TLS.EffectiveCertPath()
		key = s.cfg.TLS.EffectiveKeyPath()
		if _, errLoad := tls.LoadX509KeyPair(cert, key); errLoad != nil {
			_ = primaryListener.Close()
			if localHTTPListener != nil {
				_ = localHTTPListener.Close()
			}
			return fmt.Errorf("failed to load TLS certificate pair: %w", errLoad)
		}
	}
	serverCount := 1
	if localHTTPListener != nil {
	...
	if localHTTPListener != nil {
		log.Debugf("Starting secondary HTTP server on %s", s.localHTTPServer.Addr)
		go func() {
			if errServe := s.localHTTPServer.Serve(localHTTPListener); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				errCh <- fmt.Errorf("failed to start secondary HTTP server: %v", errServe)
				return
			}
			errCh <- nil
		}()
	...
	if useTLS {
		...
		go func() {
			if errServeTLS := s.server.ServeTLS(primaryListener, cert, key); errServeTLS != nil && !errors.Is(errServeTLS, http.ErrServerClosed) {
				errCh <- fmt.Errorf("failed to start HTTPS server: %v", errServeTLS)
				return
			}
			errCh <- nil
		}()
		...
		go func() {
			if errServe := s.server.Serve(primaryListener); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				errCh <- fmt.Errorf("failed to start HTTP server: %v", errServe)
				return
			}
			errCh <- nil
		}()
	...
	for i := 0; i < serverCount; i++ {
		if errServe := <-errCh; errServe != nil {
			return errServe
		}
	}
	return nil
...
func (s *Server) Stop(ctx context.Context) error {
	...
	if s.localHTTPServer != nil {
		if err := s.localHTTPServer.Shutdown(ctx); err != nil {
			shutdownErrs = append(shutdownErrs, fmt.Errorf("failed to shutdown secondary HTTP server: %v", err))
		}
	}
	if err := s.server.Shutdown(ctx); err != nil {
		shutdownErrs = append(shutdownErrs, fmt.Errorf("failed to shutdown HTTP server: %v", err))
	}
