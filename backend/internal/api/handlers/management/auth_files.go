...
func (h *Handler) managementCallbackURL(path string) (string, error) {
	...
	if h.cfg.LocalHTTP.Enable {
		host := config.CallbackHost(h.cfg.LocalHTTP.EffectiveHost())
		return fmt.Sprintf("http://%s:%d%s", host, h.cfg.LocalHTTP.EffectivePort(), path), nil
	}
	scheme := "http"
	if h.cfg.TLS.Enable {
		scheme = "https"
	}
	host := config.CallbackHost(h.cfg.Host)
	return fmt.Sprintf("%s://%s:%d%s", scheme, host, h.cfg.Port, path), nil
}
