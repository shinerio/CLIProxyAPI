...
// TLS
func (h *Handler) GetTLSEnable(c *gin.Context) {
	c.JSON(200, gin.H{"enable": h.cfg.TLS.Enable})
}
func (h *Handler) PutTLSEnable(c *gin.Context) {
	h.updateBoolField(c, func(v bool) { h.cfg.TLS.Enable = v })
}

func (h *Handler) GetTLSCert(c *gin.Context) {
	c.JSON(200, gin.H{"cert": h.cfg.TLS.Cert})
}
func (h *Handler) PutTLSCert(c *gin.Context) {
	h.updateStringField(c, func(v string) { h.cfg.TLS.Cert = strings.TrimSpace(v) })
}

func (h *Handler) GetTLSKey(c *gin.Context) {
	c.JSON(200, gin.H{"key": h.cfg.TLS.Key})
}
func (h *Handler) PutTLSKey(c *gin.Context) {
	h.updateStringField(c, func(v string) { h.cfg.TLS.Key = strings.TrimSpace(v) })
}
