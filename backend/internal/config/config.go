...
type Config struct {
	...
	// LocalHTTP config controls the optional secondary plain HTTP listener.
	// This is useful when the primary listener serves HTTPS for public traffic
	// while a separate HTTP socket is exposed only on LAN/private interfaces.
	LocalHTTP LocalHTTPConfig `yaml:"local-http" json:"local-http"`
	...
	// UsageStatisticsPersist enables snapshot persistence so usage statistics survive restarts.
	UsageStatisticsPersist bool `yaml:"usage-statistics-persist" json:"usage-statistics-persist"`

	// UsageStatisticsPath is the file path used to persist usage statistics snapshots.
	UsageStatisticsPath string `yaml:"usage-statistics-path" json:"usage-statistics-path"`

	// UsageStatisticsFlushIntervalSeconds controls how often persisted usage snapshots are flushed to disk.
...
// LocalHTTPConfig holds settings for an optional secondary plain HTTP listener.
type LocalHTTPConfig struct {
	// Enable toggles the additional HTTP listener.
	Enable bool `yaml:"enable" json:"enable"`
	// Host is the network host/interface for the additional HTTP listener.
	Host string `yaml:"host" json:"host"`
	// Port is the network port for the additional HTTP listener.
...
type RemoteManagement struct {
	...
	// DisableAutoUpdatePanel is deprecated and ignored.
	// Management assets are served only from local files.
	DisableAutoUpdatePanel bool `yaml:"disable-auto-update-panel"`
	// LocalFrontendPath points to a local frontend file or directory.
	// When empty, the server falls back to the local static directory under the install/config path.
	LocalFrontendPath string `yaml:"local-frontend-path"`
	// PanelGitHubRepository is deprecated and ignored.
...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	cfg.RemoteManagement.LocalFrontendPath = strings.TrimSpace(cfg.RemoteManagement.LocalFrontendPath)
