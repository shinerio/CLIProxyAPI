...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if oldCfg.LocalHTTP.Enable != newCfg.LocalHTTP.Enable {
		changes = append(changes, fmt.Sprintf("local-http.enable: %t -> %t", oldCfg.LocalHTTP.Enable, newCfg.LocalHTTP.Enable))
	}
	if strings.TrimSpace(oldCfg.LocalHTTP.Host) != strings.TrimSpace(newCfg.LocalHTTP.Host) {
		changes = append(changes, fmt.Sprintf("local-http.host: %s -> %s", strings.TrimSpace(oldCfg.LocalHTTP.Host), strings.TrimSpace(newCfg.LocalHTTP.Host)))
	}
	if oldCfg.LocalHTTP.Port != newCfg.LocalHTTP.Port {
		changes = append(changes, fmt.Sprintf("local-http.port: %d -> %d", oldCfg.LocalHTTP.Port, newCfg.LocalHTTP.Port))
	}
	...
	if oldCfg.UsageStatisticsMaxDetails != newCfg.UsageStatisticsMaxDetails {
		changes = append(changes, fmt.Sprintf("usage-statistics-max-details: %d -> %d", oldCfg.UsageStatisticsMaxDetails, newCfg.UsageStatisticsMaxDetails))
	}
	...
	} else if !reflect.DeepEqual(trimClientAPIKeys(oldCfg.APIKeys), trimClientAPIKeys(newCfg.APIKeys)) {
	...
	oldLocalFrontendPath := strings.TrimSpace(oldCfg.RemoteManagement.LocalFrontendPath)
	newLocalFrontendPath := strings.TrimSpace(newCfg.RemoteManagement.LocalFrontendPath)
	if oldLocalFrontendPath != newLocalFrontendPath {
		changes = append(changes, fmt.Sprintf("remote-management.local-frontend-path: %s -> %s", oldLocalFrontendPath, newLocalFrontendPath))
	}
...
func trimClientAPIKeys(in []config.ClientAPIKeyConfig) []string {
