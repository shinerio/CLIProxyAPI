...
func TestBuildConfigChangeDetails_SecretsAndCounts(t *testing.T) {
	oldCfg := &config.Config{
		SDKConfig: sdkconfig.SDKConfig{
			APIKeys: []sdkconfig.ClientAPIKeyConfig{{Key: "a"}},
		},
		AmpCode: config.AmpCode{
			UpstreamAPIKey: "",
		},
	...
	newCfg := &config.Config{
		SDKConfig: sdkconfig.SDKConfig{
			APIKeys: []sdkconfig.ClientAPIKeyConfig{{Key: "a"}, {Key: "b"}, {Key: "c"}},
		},
		AmpCode: config.AmpCode{
...
func TestBuildConfigChangeDetails_FlagsAndKeys(t *testing.T) {
	oldCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			...
			APIKeys:                    []sdkconfig.ClientAPIKeyConfig{{Key: "key-1"}},
	...
	newCfg := &config.Config{
		...
		RemoteManagement: config.RemoteManagement{
			DisableControlPanel:    true,
			DisableAutoUpdatePanel: true,
			LocalFrontendPath:      "/new/frontend",
		...
		SDKConfig: sdkconfig.SDKConfig{
			...
			APIKeys:                    []sdkconfig.ClientAPIKeyConfig{{Key: " key-1 "}, {Key: "key-2"}},
	...
	expectContains(t, details, "remote-management.disable-auto-update-panel: false -> true")
	expectContains(t, details, "remote-management.local-frontend-path:  -> /new/frontend")
...
func TestBuildConfigChangeDetails_AllBranches(t *testing.T) {
	oldCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			RequestLog: false,
			ProxyURL:   "http://old-proxy",
			APIKeys:    []sdkconfig.ClientAPIKeyConfig{{Key: " keyA "}},
		},
	...
	newCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			RequestLog: true,
			ProxyURL:   "http://new-proxy",
			APIKeys:    []sdkconfig.ClientAPIKeyConfig{{Key: "keyB"}},
		},
	...
	expectContains(t, changes, "remote-management.local-frontend-path:  -> /new/frontend")
