...
func main() {
	...
	flag.BoolVar(&localModel, "local-model", false, "Deprecated: model catalog is always loaded from local embedded data")
	...
	if vertexImport != "" {
		...
		if localModel && (!tuiMode || standalone) {
			log.Info("Local model mode is now the default: using embedded model catalog only")
		}
		if tuiMode {
			if standalone {
