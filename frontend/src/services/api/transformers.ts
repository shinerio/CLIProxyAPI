...
const normalizeClientApiKeys = (input: unknown): ClientApiKeyConfig[] => {
  ...
  input.forEach((item) => {
    ...
    const keyRaw =
      typeof item === 'string'
        ? item
        : record
          ? (record.key ?? record['api-key'] ?? record.apiKey ?? record.value)
          : '';
...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  if (Array.isArray(apiKeysRaw)) {
    config.apiKeys = normalizeClientApiKeys(apiKeysRaw);
  }
